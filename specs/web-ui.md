...
| `/recaps/[slug]` | Event recap pages (`EventRecap`: summary; optional YouTube presentation grid with metadata from YouTube player response via `getRecapYoutubePresentationCards`; non-YouTube `videoUrl` stays a single embed; optional `interviews[]`, gallery). When `videoUrl` parses as YouTube, Open Graph + JSON-LD `image` use `i.ytimg.com/.../hqdefault.jpg`. |
