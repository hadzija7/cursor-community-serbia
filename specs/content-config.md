...
| `content/hackathon.ts` | Hackathon event copy (title: Grok Bot Serbia Hackathon), full-circle Grok Bot mark (`/grokbot.svg`), Overview animated ink orb (`/bloub-cercle-neutre-encre-anime.svg`), Luma URL, prize tracks, hacker guide, mentors/hosts/judges, sponsor logos, community partners, stack profiles (including `mcp` install config) |
