...
const accentStyles = {
  orange: {
    panel: 'border-cursor-accent-orange/30 bg-cursor-accent-orange-bg/50',
    text: 'text-cursor-accent-orange',
  },
  green: {
    panel: 'border-cursor-accent-green/30 bg-cursor-accent-green-bg/50',
    text: 'text-cursor-accent-green',
  },
  yellow: {
    panel: 'border-cursor-accent-yellow/30 bg-cursor-accent-yellow-bg/50',
    text: 'text-cursor-accent-yellow',
  },
  purple: {
    panel: 'border-cursor-accent-purple/30 bg-cursor-accent-purple-bg/50',
    text: 'text-cursor-accent-purple',
  },
  blue: {
    panel: 'border-cursor-accent-blue/30 bg-cursor-accent-blue-bg/50',
    text: 'text-cursor-accent-blue',
  },
} as const
...
function SponsorCard({
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="w-full max-w-sm rounded-xl border border-cursor-border-emphasis bg-cursor-surface-raised/80 p-4 text-left transition-colors hover:border-cursor-accent-orange/40"
    >
      <div
        className="mb-3 inline-flex items-center rounded-md px-3 py-2"
        style={{ backgroundColor: profile.logoBg ?? '#14120b' }}
      >
        <SponsorLogo
          profile={profile}
          className={`${profile.logoHeight ?? 'h-7'} w-auto max-w-[8rem] object-contain`}
        />
      </div>
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 id="sponsor-modal-title" className="mt-2 text-2xl font-semibold tracking-tight">
              {profile.name}
            </h2>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        {groups.map(({ stage, profile }) => {
          ...
          return (
            <section
              key={stage.id}
              className={`min-h-[16rem] rounded-2xl border p-6 md:p-8 ${styles.panel}`}
            >
              <p className={`text-xs font-semibold uppercase tracking-[0.18em] ${styles.text}`}>{stage.label}</p>
              <h2 className="mt-2 text-2xl font-semibold tracking-tight">{stage.job}</h2>
              <div className="mt-6 flex flex-wrap gap-4">
                <SponsorCard profile={profile} onOpen={() => setOpenId(profile.id)} />
              </div>
              {stage.id === 'experience' ? (
                <p className="mt-6 max-w-sm text-xs leading-relaxed text-cursor-text-muted">
